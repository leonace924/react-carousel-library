...
import { CarouselContainer, Div, NextButton, PrevButton } from "./Carousel.styled";
...
const Carousel: React.FC<CarouselProps> = ({
  ...
  const adjustCarouselWrapperPosition = useCallback(
    (ref: HTMLDivElement) => {
      ...
      const getDefaultPositionValue = () => getSlideValue() * childArray.length;
      const setPosition = (v: number) => {
        ref.style.top = "0px";
        ref.style.left = "0px";
        ref.style[positionProperty] = `-${v}px`;
      };
      const setStartPosition = () => setPosition(getDefaultPositionValue());
  ...
  useMount(() => {
    if (initialActiveItem > 0 && initialActiveItem <= childArray.length) {
  ...
  const getIsLastItem = () => getCurrentActiveItem() === childArray.length - 1;
  ...
  const slideToPrevItem = () => {
    if ((!isInfinite && getCurrentActiveItem() === 0) || windowIsHidden.current)
      return;
  ...
  const slideToNextItem = () => {
    if (
      (!isInfinite && getCurrentActiveItem() === internalChildren.length - 1) ||
